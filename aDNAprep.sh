...
#Note that the reference genome is in a different directory called source
freebayes -f source/GRCh37-lite.fa -r 1 $file > $id.vcf
...
#Note that the reference vcf and map are in a different directory called source
beagle gp=true impute=true gt=$id.vcf ref=source/chr1.1kg.phase3.v5a.vcf.gz map=source/plink.chr1.GRCh37.map out=$out
